<div>
    ...
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 relative overflow-hidden bg-gradient-to-br from-emerald-700/20 via-slate-900 to-slate-950 border {{ $writtenToday ? 'border-emerald-500/30' : 'border-emerald-600/40' }} rounded-xl p-8 shadow-lg">
                ...
                @if($writtenToday)
                    ...
                    <div class="relative">
                        <div class="flex items-center gap-2 text-emerald-400 mb-3 ds-eyebrow">
                        ...
                        <p class="ds-body text-slate-400 mb-5">You've already completed your writing challenge. Come back tomorrow for a new prompt, or do another session for extra practice.</p>
                ...
                @else
                    ...
                    <div class="relative">
                        ...
                        <a href="{{ route('writing-coach') }}" class="ds-btn ds-btn-lg ds-btn-primary">
            ...
            <div class="flex flex-col gap-4">
                {{-- Streak — intentional personality card, keep orange --}}
                <div class="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-xl p-6 flex items-center justify-between shadow relative overflow-hidden group flex-1">
                    ...
                    <div class="z-10">
                        <p class="ds-eyebrow mb-1">Streak</p>
                        <div class="flex items-baseline gap-1">
                            <span class="text-4xl font-black text-orange-500">{{ $streak }}</span>
                            <span class="ds-muted font-medium">days</span>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="ds-card p-6">
                ...
                @if(!$scoreDeltas)
                    <div class="text-center py-6">
                        <p class="ds-body text-slate-500">No writing session today yet.</p>
                        <p class="ds-muted mt-1">Complete today's challenge to see your score deltas here.</p>
                    </div>
                @else
                    @php
                        $scoreLabels = [
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    @if($hasYesterday)
                                        ...
                                        @endif
                                    @else
                                        <span class="ds-muted w-10">first</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="mt-4 pt-4 border-t border-slate-800 flex items-center justify-between">
                        <span class="ds-muted">CEFR estimate</span>
                        {{-- CEFR badge: slate (informational/neutral), not indigo --}}
                        <x-ui.badge>{{ $scoreDeltas['cefr'] }}</x-ui.badge>
                    </div>
                @endif
            </div>
            ...
            <div class="ds-card p-6">
                <h3 class="ds-eyebrow mb-5 flex items-center gap-2">
                    <svg class="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                    Weakness of the Month
                </h3>

                @if(!$weekTopWeakness)
                    <div class="text-center py-6">
                        <p class="ds-body text-slate-500">No mistakes tracked yet.</p>
                        <p class="ds-muted mt-1">Submit Writing Coach sessions to start tracking weaknesses.</p>
                    </div>
                @else
                    @php
                        $weakColors = [
                            'Grammar'    => ['ring' => 'ring-red-500/30',   'text' => 'text-red-400',   'bg' => 'bg-red-500/10'],
                            'Vocabulary' => ['ring' => 'ring-amber-500/30', 'text' => 'text-amber-400', 'bg' => 'bg-amber-500/10'],
                            'Writing'    => ['ring' => 'ring-slate-500/30', 'text' => 'text-slate-400', 'bg' => 'bg-slate-500/10'],
                        ];
                    ...
                    <div class="flex flex-col items-center justify-center py-4 text-center">
                        ...
                        <p class="ds-body text-slate-400 mb-4">{{ $weekTopWeakness->total }} mistake{{ $weekTopWeakness->total > 1 ? 's' : '' }} in the last 30 days</p>
                        <p class="ds-muted leading-relaxed">
        ...
        <div class="ds-card p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="ds-eyebrow">Weekly Snapshot</h3>
