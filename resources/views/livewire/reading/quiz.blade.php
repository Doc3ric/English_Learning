<div class="max-w-4xl mx-auto">
    ...
    @if(!$isSubmitted)
        <div class="mb-6 flex justify-between items-center" x-data="readingTimer({{ $elapsedSeconds }})">
            <h2 class="ds-section-title text-xl">Test Your Comprehension</h2>
            ...
            @foreach($questions as $index => $q)
                <div class="ds-card p-6">
                    ...
                    @if($q->question_type === 'multiple_choice')
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 p-3 rounded-lg border border-slate-800 hover:border-emerald-500/50 cursor-pointer transition-colors bg-slate-900/50 has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-900/10">
                    ...
                    @elseif($q->question_type === 'true_false_not_given')
                        <div class="flex gap-4">
                            <label class="flex-1 text-center p-3 rounded-lg border border-slate-800 hover:border-emerald-500/50 cursor-pointer transition-colors bg-slate-900/50 has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-900/20">
                    ...
                    @elseif($q->question_type === 'short_answer')
                        <input type="text" wire:model="answers.{{ $q->id }}" class="ds-input font-bold" required placeholder="Type your answer...">
                    @endif
                </div>
            @endforeach
            
            <div class="pt-6 flex justify-end">
                <button type="submit" class="ds-btn ds-btn-lg ds-btn-primary rounded-full px-12 w-full md:w-auto">
                    Submit Answers
                </button>
            </div>
        </form>
    ...
    @else
        <!-- Results Section -->
        <div class="mb-8 ds-card text-center !p-8">
            <h2 class="text-3xl font-bold text-slate-100 mb-2">Practice Completed</h2>
            
            <div class="flex flex-wrap justify-center gap-6 mt-6">
                <div class="ds-card-nested px-6 py-4">
                    <p class="ds-eyebrow mb-1">Score</p>
                ...
                <div class="ds-card-nested px-6 py-4">
                    <p class="ds-eyebrow mb-1">Time Taken</p>
                ...
                @if($wpm)
                <div class="ds-card-nested px-6 py-4">
                    <p class="ds-eyebrow mb-1">Reading Speed</p>
                    <p class="text-3xl font-bold text-slate-200">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <h3 class="ds-section-title text-xl border-b border-slate-800 pb-2">Review Your Answers</h3>
                
                @foreach($questions as $index => $q)
                    @php $res = $results[$q->id]; @endphp
                    <div class="ds-card {{ $res['is_correct'] ? '!border-emerald-500/30' : '!border-red-500/30' }}">
                        ...
                        <div class="text-sm space-y-2 mb-3">
                            <p class="ds-muted">Your Answer: <span class="{{ $res['is_correct'] ? 'text-emerald-400' : 'text-red-400' }} font-bold">{{ $res['user_answer'] ?: 'None' }} {!! $res['is_correct'] ? '&#10003;' : '&#10007;' !!}</span></p>
                            
                            @if(!$res['is_correct'])
                                <p class="ds-muted">Correct Answer: <span class="text-emerald-400 font-bold">{{ $res['correct_answer'] }}</span></p>
                            @endif
                        </div>
                ...
                <div class="mt-12">
                    <h3 class="ds-section-title text-xl border-b border-slate-800 pb-2 mb-6">Write a Summary (IELTS Task 2 Practice)</h3>
            ...
            <div class="space-y-6">
                ...
                <h3 class="ds-section-title text-xl border-b border-slate-800 pb-2">Extract Vocabulary</h3>
                <p class="ds-muted text-sm mb-4">Did you encounter any new words in the text? Add them to your vocabulary now.</p>
                @livewire('reading.vocabulary-add', ['articleId' => $article->id])
                
                <div class="mt-8 pt-6 border-t border-slate-800 text-center">
                    <a href="{{ route('reading') }}" class="ds-muted hover:text-emerald-400 transition-colors underline">Back to Reading Tracker</a>
                </div>
            </div>
        </div>
    @endif
