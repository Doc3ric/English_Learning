<div class="max-w-3xl mx-auto">
    ...
    @if(!$submitted)
        <div class="ds-card p-8">
            <h3 class="ds-section-title mb-6">Test Your Knowledge</h3>
            ...
            <form wire:submit="submitQuiz" class="space-y-8">
                @foreach($lesson->questions as $index => $question)
                    <div class="ds-card-nested p-4">
                        <p class="font-medium text-slate-200 mb-4">
                            <span class="text-emerald-500 font-bold mr-2">{{ $index + 1 }}.</span>
                            {{ $question->question }}
                        </p>
                ...
                <div class="pt-4 text-center">
                    {{-- "Submit Answers" rounded-full → ds-btn-primary (flagged fix) --}}
        ...
        <div class="ds-card p-8">
            ...
            <div class="text-center mb-10 pb-10 border-b border-slate-800">
                @if($passed)
                    ...
                    <p class="ds-muted mb-8">You've unlocked the next lesson on your roadmap.</p>
                    {{-- "Continue" rounded-full → ds-btn-primary (flagged fix) --}}
                    <a href="{{ route('grammar') }}" class="ds-btn ds-btn-lg ds-btn-primary inline-flex">
                ...
                @else
                    ...
                    <p class="ds-muted mb-8">You need 70% or higher to pass. Review the material and try again.</p>
                    {{-- "Retry" rounded-full → ds-btn-secondary (flagged fix) --}}
                    <button wire:click="retry" class="ds-btn ds-btn-lg ds-btn-secondary">
                        Retry Quiz
                    </button>
                @endif
            </div>
            ...
            <div class="space-y-6">
                @foreach($lesson->questions as $index => $question)
                    ...
                    <div class="p-4 border {{ $isCorrect ? 'border-emerald-500/30 bg-emerald-900/10' : 'border-red-500/30 bg-red-900/10' }} rounded-xl">
                        <p class="font-medium text-slate-200 mb-4">
                            <span class="{{ $isCorrect ? 'text-emerald-500' : 'text-red-500' }} font-bold mr-2">{{ $index + 1 }}.</span>
                            {{ $question->question }}
                        </p>
                        ...
                        @if($question->explanation)
                            <div class="pt-3 border-t {{ $isCorrect ? 'border-emerald-500/20' : 'border-red-500/20' }}">
                                <p class="ds-body"><span class="font-semibold text-slate-400">Explanation:</span> {{ $question->explanation }}</p>
                            </div>
                        @endif
                    </div>
                @endforeach
